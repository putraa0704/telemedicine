...
<script>
    ...
    async function loadKonsultasi() {
        try {
            var res  = await fetch('/api/konsultasi/saya?_t=' + new Date().getTime(), { headers: { 'Authorization': 'Bearer ' + token }, cache: 'no-store' });
    ...
    async function kirimPesanAktif() {
        ...
        try {
            ...
            if (res.ok) {
                ...
                var resList = await fetch('/api/konsultasi/saya?_t=' + new Date().getTime(), { headers: { 'Authorization': 'Bearer ' + token }, cache: 'no-store' });
